...
class Github_Updater {
	...
	/**
	 * Plugin basename (e.g. "site-walker-wp/site-walker-wp.php").
	 *
	 * @var string
	 */
	...
	public function __construct() {
		$this->plugin_basename = plugin_basename( \STWLK_PLUGIN_FILE );
	...
	private function is_enabled(): bool {
		/**
		 * Filter whether GitHub auto-updates are enabled for Site Walker.
		 *
		 ...
		 */
		return (bool) apply_filters( 'site_walker_updater_enabled', true );
	}
	...
	public function check_for_update( $transient ) {
		...
		if ( empty( $checked ) ) {
			...
			} elseif ( version_compare( \STWLK_PLUGIN_VERSION, $release['version'], '>=' ) ) {
				$this->log( 'check_for_update: current version ' . \STWLK_PLUGIN_VERSION . ' is up to date (latest: ' . $release['version'] . ').' );
			} else {
				$this->log( 'check_for_update: update available ' . \STWLK_PLUGIN_VERSION . ' → ' . $release['version'] . '.' );
	...
	public function plugin_info( $result, $action, $args ) {
		...
		if ( is_array( $release ) ) {
			...
			$plugin_data = get_plugin_data( \STWLK_PLUGIN_FILE, false, true );
	 ...
	 * (e.g. "site-walker-wp.zip" or "site-walker-wp-1.0.0.zip").
	...
	private function log( string $message ): void {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( 'Site_Walker Github_Updater: ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Intentional debug logging.
		}
	}
	...
	private function log_error( string $message ): void {
		error_log( 'Site_Walker Github_Updater [error]: ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Intentional error logging for updater failures.
	}
}
